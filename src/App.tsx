...
function App() {
  ...
  useEffect(() => {
    ...
    const discoveredExperiments = Object.entries(experimentReadmes).map(([path, content]) => {
      const match = path.match(/experiment-([^/]+)/)
      const experimentName = match ? match[0] : ''
      
      // Parse title and description from README
      const lines = (content as string).split('\n')
      const title = lines.find((line: string) => line.startsWith('# '))?.replace('# ', '') || experimentName
      const description = lines.find((line: string) => line.trim() && !line.startsWith('#')) || ''
